...
|---|---|---|---|---|
| 1 | `tests/test_exports.py::isolated_exports_dir` post-fix shape | Post-fix, `isolated_exports_dir` is functionally equivalent to the conftest `db` fixture except for its return value (the `tmp_path / "exports"` path that smoke tests need). Two paths to the same isolation pattern in the same suite is a minor non-DRY signal. Lifting `isolated_exports_dir` into conftest as a thin wrapper around `db` (the same shape `db_and_exports` already uses) would consolidate; today the implementer's choice keeps it page-local because consumers are all within `test_exports.py`. | ℹ️ | Defer. If Phase 6 T4 / T5 (Export page tests) want the fixture, lift then; today single-file consumer = local. |
| 2 | Process | The CI-mirror local check (`mv postdoc.db postdoc.db.bak && pytest && mv postdoc.db.bak postdoc.db`) — codified in `c284c20` — would have caught this gap pre-push if the gate had been in `AGENTS.md` before T1 started. Implementer's `fix:` commit confirms the gate now does its job: the local check passes 815 + 1 xfailed with no `postdoc.db` at the project root, matching CI. | ℹ️ | The gate is now standing for every future PR. |

*No 🔴 / 🟠 / 🟡 / 🟢 findings.*

*Two additional kept-by-design choices (no `notes` column in `RECOMMENDERS.md`, deadline-merge from second SQL reader rather than amending `get_all_recommenders()` SQL) are addressed in the Q&A section per `GUIDELINES §10` ("`Kept by design` observations belong in the Q&A section, not in the Findings table — they are not defects"). See Q1 (notes column omission rationale) and Q2 (deadline-merge vs SQL amendment).*
