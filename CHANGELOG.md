...
### Added
- Public Streamlit Cloud demo at `academic-application-tracker.streamlit.app` — per-session in-memory SQLite keeps concurrent visitors fully isolated (no cross-session data leakage) (#111)
- Demo Mode banner + sidebar "Reset demo data" button rendered on every page when `AAT_DEMO=1` (#111)
- 18-position multi-cycle seed dataset covering all 7 application statuses (SAVED, APPLIED, INTERVIEW, OFFER, CLOSED, REJECTED, DECLINED) so every alert panel surfaces something on first render (#111)
- `.streamlit/config.toml` for the Streamlit Community Cloud deploy (#111)
...
- Trim recent CHANGELOG entries to GUIDELINES §14.4 one-line shape; add bottom link references; restructure phase-6-tier5 + phase-7-tier1 review docs to move `Kept by design` rows from Findings table to Q&A per §10. (`5f1d0f3`)
