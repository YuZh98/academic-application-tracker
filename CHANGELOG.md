...
### Added
- Phase 6 T5: `pages/4_Export.py` extended with three `st.download_button` widgets (one per locked filename, in wireframe order) + the wireframe-pinned "── Download ───" section header (rendered as `st.divider()` + `st.subheader("Download")` — Streamlit's markdown doesn't render Unicode dashes as a horizontal rule, so the divider+subheader pair is the idiomatic equivalent). Each button: `data=Path.read_bytes()` + `disabled=False` when the file exists; `data=b""` + `disabled=True` when absent. A single `_file_present = _path.exists()` boolean drives BOTH the new download-disabled branch AND the existing T4 mtime-line branch — single filesystem check per file. Stacked layout (button above mtime line) rather than wireframe's side-by-side `st.columns` because the column layout would have moved the bold filename onto the button label and broken T4's `TestExportPageMtimesPanel` substring assertions. 7 new tests in `TestExportPageDownloadButtons` + two helpers (`_download_buttons` wrapping `at.get('download_button')` since AppTest 1.56 has no typed accessor; `_download_button` looking up by widget key via `proto.id.endswith(...)`). Two source-grep tests pin the `data=read_bytes()` + `file_name=` contracts because AppTest 1.56's DownloadButton proto doesn't expose either field (both stored behind a mock media URL); the integration `test_download_button_enabled_when_file_present` is the belt-and-suspenders pin. Suite 827 → 834 under both pytest gates. Merged via PR #36 (`73a04c4`). ([`reviews/phase-6-tier5-review.md`](reviews/phase-6-tier5-review.md))

### Changed
- **Privacy amendment — `exports/` is `.gitignore`d, not committed.** DESIGN §7 contract #2 originally read "Export formats committed into version control"; amended 2026-05-04 to privacy-first. The rendered markdown carries personal job-search data (names, deadlines, application status across institutions) that doesn't belong in a public repo. Schema and code stay versioned; user data — both `postdoc.db` and the markdown the writers regenerate from it — stay user-local. `.gitignore` now covers `postdoc.db` AND `exports/` under one "Personal data" comment block. DESIGN §7 contract #2's wording shifts from "committed into version control" to "deterministic and idempotent" with the privacy carve-out cited inline. No code change — `exports.py` writers, the regenerate button, and the auto-write hook all behave identically; only the question of what's tracked in git changed. The user's local `exports/` directory is the durable surface; the Export page's regenerate button + every `database.py` writer's deferred-import call keep it fresh.

### Added
- Phase 6 T4: `pages/4_Export.py` Export page — page shell (`set_page_config(layout="wide")`, `database.init_db()`, `st.title("Export")`, verbatim wireframe-pinned intro line via `st.markdown`) + manual regenerate button (`st.button("Regenerate all markdown files", key="export_regenerate", type="primary")` wrapping `exports.write_all()` in try/except per GUIDELINES §8 — friendly `st.error(f"Could not regenerate: {e}")` on failure with the button still rendered for retry, no re-raise; success path fires `st.toast("Markdown files regenerated.")`) + per-file mtimes panel (one `st.markdown` line per locked filename `OPPORTUNITIES.md` / `PROGRESS.md` / `RECOMMENDERS.md` in wireframe order, either `**{filename}** — last generated: {YYYY-MM-DD HH:MM:SS}` or `**{filename}** — not yet generated`; `Path.exists()` check + `datetime.fromtimestamp(.stat().st_mtime).strftime(...)` formatting). The "── Download ───" wireframe section header is deliberately omitted from T4 — that header scopes T5's download buttons. 12 new tests in 3 classes (`TestExportPageShell`, `TestExportPageRegenerateButton`, `TestExportPageMtimesPanel`) with all locked-copy strings centralized in module-level constants for one-edit drift surface; mtimes test uses `os.utime` to a deterministic epoch (`1700000000`) so timestamp assertions are exact rather than wall-clock-approximated. Suite 815 → 827 under both pytest gates. Merged via PR #35 (`3235f60`). First project PR to land with `mergeStateStatus: CLEAN`. ([`reviews/phase-6-tier4-review.md`](reviews/phase-6-tier4-review.md))
- Phase 6 T3: `exports.write_recommenders()` markdown generator — fills the third `exports.py` stub. Reads `database.get_all_recommenders()` (recommenders × positions LEFT JOIN ordered by `recommender_name ASC`) and writes an 8-column markdown table to `exports/RECOMMENDERS.md`: Recommender · Relationship · Position · Institute · Asked · Confirmed · Submitted · Reminder. **`notes` deliberately omitted** — free-form prose is awkward in a markdown table cell; the in-app UI carries it. Two helper-reuse decisions: **Confirmed** uses a NEW local `_format_confirmed` (`—` / `No` / `Yes` tri-state — mirrors the `pages/3_Recommenders.py` helper of the same name; pages and exports cannot share helpers per DESIGN §2 layer rules); **Reminder** REUSES the existing `exports._format_confirmation` helper because the `(reminder_sent, reminder_sent_date)` pair has the same `(flag, date)` shape as the Applications-page Confirmation pattern (DESIGN §8.3 D-A T1-C precedent). Cell shapes mirror T1+T2 (em-dash for missing TEXT, ISO TEXT pass-through for dates, `_md_escape_cell` on every cell). Sort `recommender_name ASC, deadline_date ASC NULLS LAST, id ASC`; `deadline_date` is merged in pandas from `database.get_all_positions()` (T2 "compose multiple reads in `exports.py`" precedent). Idempotent (DESIGN §7 contract #2). 14 new tests in `TestWriteRecommenders`; suite 801 → 815 under both pytest gates. Merged via PR #34 (`c11fde4`). ([`reviews/phase-6-tier3-review.md`](reviews/phase-6-tier3-review.md))

### Fixed
- `tests/test_exports.py::isolated_exports_dir` augmented to monkeypatch BOTH `exports.EXPORTS_DIR` AND `database.DB_PATH` + run `init_db()` (was: only `EXPORTS_DIR`). Closes the CI-red regression that had been latent on main since the Phase 6 T1 merge: three smoke tests at the top of `tests/test_exports.py` (`test_write_*_does_not_raise`) called the writers, which post-T1 read the DB; without `DB_PATH` monkeypatching the unpatched `database.DB_PATH` fell through to the developer's real `postdoc.db` locally (silent pass) and raised `sqlite3.OperationalError: no such table: positions` / `recommenders` on CI runners. PRs #32 + #33 admin-bypass-merged while CI was IN_PROGRESS; both eventually completed FAILURE. Mirror of the conftest `db` fixture from Phase 6 T2 — single fixture, every consumer (the three smoke tests + the two `write_all` behaviour tests) gets full DB + exports isolation. Phase 6 T3 ride-along; merged via PR #34 (`c11fde4`).

### Changed
- `ORCHESTRATOR_HANDOFF.md` "Branch-protection note" + `AGENTS.md` "Session bootstrap" — codify CI-green-conclusion-before-admin-bypass + the CI-mirror local check (`mv postdoc.db postdoc.db.bak && pytest tests/ -q && mv postdoc.db.bak postdoc.db`). The handoff doc now requires `gh pr checks <N> --watch` + `gh pr view <N> --json statusCheckRollup` showing `conclusion: SUCCESS` before admin-bypass; `IN_PROGRESS` / `QUEUED` / `null` is NOT acceptable. The bootstrap doc adds the CI-mirror check to the standing pre-PR checklist. Both changes land in `c284c20`. Post-mortem on PRs #32–#34 — main shipped CI-red across two tier merges before PR #34's CI surfaced the same failure and the orchestrator caught it.

### Added
- Phase 6 T2: `exports.write_progress()` markdown generator — fills the second `exports.py` stub. Renders a single markdown table to `exports/PROGRESS.md` from positions × applications × interviews (`database.get_applications_table()` for the positions+applications LEFT JOIN + per-row `database.get_interviews(position_id)` for interviews). 8-column contract pinned by `TestWriteProgress.EXPECTED_HEADER`: Position · Institute · Status · Applied · Confirmation · Response · Result · Interviews. **No `Deadline` column** — the two exports answer different questions (OPPORTUNITIES.md owns the upstream window; PROGRESS.md owns the application progression). Cell shapes mirror T1: em-dash for missing TEXT via `_safe_str_or_em`; ISO TEXT pass-through for date cells; raw bracketed status sentinel (`[APPLIED]` etc.); `_md_escape_cell` on every cell. Two new tri-state helpers: `_format_confirmation(received, iso_date)` mirrors the Applications page DESIGN §8.3 D-A T1-C inline-text shape (`—` / `✓ {ISO}` / `✓ (no date)`) using ISO over Mon D so the export round-trips; `_format_interviews_summary(scheduled_dates)` renders `"{N} (last: {YYYY-MM-DD})"` with `last` = max scheduled_date — round-trippable + deterministic + idempotent (considered+rejected: count-only, comma-joined date list, next-interview cell). Sort `deadline_date ASC NULLS LAST, position_id ASC` inherited from `database.get_applications_table()` SQL ORDER BY; re-applied via `pandas.sort_values(... kind="stable")`. Per-row N+1 interviews lookup is kept-by-design (low position counts, single-user app). Idempotent (DESIGN §7 contract #2). 15 new tests in `TestWriteProgress`; suite 786 → 801 under both pytest gates. Merged via PR #33 (`911115a`). ([`reviews/phase-6-tier2-review.md`](reviews/phase-6-tier2-review.md))
- Phase 6 T1: `exports.write_opportunities()` markdown generator — fills the existing `exports.py` stub. Renders a single markdown table to `exports/OPPORTUNITIES.md` from `database.get_all_positions()`, sorted `deadline_date ASC NULLS LAST, position_id ASC` via `pandas.sort_values(... kind="stable")` (the `position_id` tiebreaker mirrors `database.get_applications_table()`). 8-column contract pinned by `TestWriteOpportunities.EXPECTED_HEADER`: Position · Institute · Field · Deadline · Priority · Status · Created · Updated. Cell shapes: `_safe_str_or_em()` coerces None / NaN / `""` → em-dash; date / datetime cells pass-through ISO TEXT verbatim; **Status renders the raw bracketed sentinel** (`[SAVED]` / `[APPLIED]` / …) NOT `STATUS_LABELS` translation — markdown is a backup format, not a UI surface, and round-trippable / greppable raw form trumps UI-friendly translation; the pre-PR status-literal grep in GUIDELINES §11 is scoped to `app.py + pages/`, not `exports/`, so this divergence stays grep-clean. `_md_escape_cell` escapes `|` and collapses `\n` / `\r`. Deferred `database` import inside the function body breaks the `database → exports → database` circular import. `EXPORTS_DIR.mkdir(exist_ok=True)` inside the writer keeps it callable independently of `write_all`'s prior mkdir (required for the Phase 6 T4 manual-trigger button). Idempotent — two calls with the same DB state produce byte-identical output; load-bearing for DESIGN §7 contract #2. New combined `db_and_exports` test fixture monkeypatches both `database.DB_PATH` and `exports.EXPORTS_DIR` to avoid polluting the project's real `exports/` via `add_position`'s deferred-import side effect. 9 new tests in `TestWriteOpportunities`; suite 777 → 786 under both pytest gates. Merged via PR #32 (`e9a8a4a`). ([`reviews/phase-6-tier1-review.md`](reviews/phase-6-tier1-review.md))

### Changed
- `tests/conftest.py::db` fixture lifted to monkeypatch BOTH `database.DB_PATH` AND `exports.EXPORTS_DIR` (was: only `DB_PATH`). Closes the test-isolation pollution exposed by Phase 6 T1: every test that called a `database.py` writer fired `exports.write_all()` via deferred import, which (post-T1) wrote to the project's real `exports/OPPORTUNITIES.md` as a side effect. Five migration-test sites in `tests/test_database.py` that opt out of the `db` fixture for hand-rolled `tmp_path` setup get paired EXPORTS_DIR monkeypatches (2 actual polluters + 3 defensive symmetry); two `write_all` behaviour tests in `tests/test_exports.py` gain the `isolated_exports_dir` fixture parameter. New T2 isolation gate (`git status --porcelain exports/` empty post-pytest) added to the standing GUIDELINES §11 pre-PR checklist. `tests/test_exports.py::db_and_exports` collapses to a thin wrapper that returns the path. Phase 6 T2 mandatory ride-along; merged via PR #33 (`911115a`).
