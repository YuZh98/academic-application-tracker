...
Sub-tasks 1–7 shipped (config additions, `REQUIREMENT_VALUES` Y/N → Yes/No
...
and `positions.work_auth_note TEXT` column + Overview-tab
`work_auth`/`work_auth_note` selectbox+text_area pair per §6.2 + §8.2 +
D22). 319 tests green · zero deprecation warnings.

**Next up:** remaining v1.3 alignment items (schema overhauls incl.
interviews sub-table, confirmation_* and reminder_sent_* splits,
`recommenders.confirmed` INTEGER, cascade R1/R2/R3 rewire), then
