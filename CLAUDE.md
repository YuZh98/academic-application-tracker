...
## Project State
**Phase:** Phase 4 **T1 merged, T2-A complete** — T1 shipped via PR #4 into `main` (HEAD `f49ec5f`); T2-A (Plotly horizontal bar funnel built from `count_by_status()`) landed on `feature/phase-4-Tier2-ApplicationFunnel`. Plan locked in `PHASE_4_GUIDELINES.md` (6 tiers, ~9 sessions, ~9.5 hr; critical path linear). 8 design decisions closed 2026-04-20; Tracked-bucket semantics + config.py carve-out locked 2026-04-21; Next-Interview format + selection rule locked 2026-04-21; empty-DB hero trigger operationalized as `tracked + applied + interview == 0` (2026-04-21). **252 tests passing, zero deprecation warnings.** Next: T2-B (empty-DB funnel branch — descriptive text when no positions), then T2-C (wrap funnel in left half of `st.columns(2)` per U2).
**Git:** on `feature/phase-4-Tier2-ApplicationFunnel` branched off `main` at `f49ec5f`. T2-A commits this session: `test(phase-4-t2a)` → `feat(phase-4-t2a)` → `chore(phase-4-t2a)`.
