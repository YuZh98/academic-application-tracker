...
- 2026-05-06 — **`v0.10.0` tagged — public-launch release** (close-out commit + annotated tag). Bundles four worker-shipped polish PRs spawned via `/batch` skill: PR #51 (`chore(ci): expand Python matrix to 3.11–3.14`, `f24a1e8`), PR #49 (`chore(test): add pytest-cov + coverage config`, `f49a5d9` — coverage at 97%), PR #48 (`docs(security): add SECURITY.md`, `1fb9978`), PR #50 (`docs(readme): add CI / Python / license badges`, `e749f74`). Plus two orchestrator-direct artefacts: `26dbde5` Phase 7 T5 responsive-layout screenshots × 20 PNGs at 1024 / 1280 / 1440 / 1680 across all five pages (captured against an empty SQLite DB to avoid leaking user tracking data; `mv postdoc.db postdoc.db.user-backup` + restart Streamlit + drive Chrome DevTools MCP + restore); `ce4ec5a` README Dashboard hero embed + coverage badge inline with the existing three. Cohesion-smoke at [`reviews/v0.10.0-finish-cohesion-smoke.md`](../../reviews/v0.10.0-finish-cohesion-smoke.md). All four public-launch ship criteria met. **`v0.10.0` is the project's public-launch release; no `v1.0.0` planned** — pre-1.0 SemVer convention (each minor = one phase) maps to release cadence cleanly. **Next step:** user-driven `gh repo edit --visibility public` flip; not part of any tag. Three doc-drift carryover commits from v0.9.0 close-out (`8ab7d3d` + `5809e68` + `1275a08`) absorbed into v0.10.0 CHANGELOG block. Suite stable at 883 passed + 1 xfailed; pyright 0/0; coverage 97%; all seven pre-tag gates green at HEAD.
...
_Updated: 2026-05-06 (`v0.10.0` tagged — public-launch release; bundles 4 worker PRs + T5 screenshots + README hero + coverage badge; suite 883 / 1 xfailed; pyright 0/0; coverage 97%. **No `v1.0.0` planned**; user-driven `gh repo edit --visibility public` flip is the next manual step)_
