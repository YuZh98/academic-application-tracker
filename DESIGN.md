...
2. **Stable markdown format.** Export formats committed into version control; output format changes documented in CHANGELOG alongside any generator change.
...
- **Reminder helpers** (per recommender card): two affordances — quick mailto for simple case, LLM-prompt expander for users who want richer drafted email.
  - **Primary `Compose reminder email` button** opens `mailto:` URL w/ locked, professional copy:
    - Subject: `Following up: letters for N postdoc applications` (where `N` = position count for that recommender)
